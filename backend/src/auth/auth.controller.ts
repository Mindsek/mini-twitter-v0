...
export class AuthController {
  ...
  async login(@Body() authBody: AuthBodyDto, @Res() res: Response) {
    ...
    try {
      ...
      this.logger.error(
        `Login failed for email: ${authBody.email} - Error: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      throw error;
    }
  }
  ...
  async register(@Body() body: AuthCreateUserDto, @Res() res: Response) {
    ...
    try {
      ...
      this.logger.error(
        `Registration failed for email: ${body.email}, username: ${body.username} - Error: ${error instanceof Error ? error.message : 'Unknown error'}`,
      );
      throw error;
    }
  }
